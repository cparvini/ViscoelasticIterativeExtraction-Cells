...
originalPath = uigetdir(pwd,'Select the Directory to Analyze');
...
for i_dir = 1:length(Folders)
    %% Load the directory for this loop
    % Start with a clean slate
    clearvars -except i_dir Folders originalPath savePrependIn N_workers
    clc
    ...
    pathname = Folders{i_dir};
    temp = strsplit(pathname,{filesep});
    temp = strsplit(temp{end},' ');
    savePrepend = [savePrependIn sprintf('Cell%s',temp{end})];
    ...
    n_samples = 1000;                       % INTEGER, The number of random-starts to attempt per model configuration
    ...
    avAnalysis = 'i';                       % ('a') Use the AVERAGE of all force curves with the SAME APPROACH VELOCITY for fitting; ('i') Process/fit files INDIVIDUALLY
    ...
    fitLog = 0;                             % (1) Fit using Log-Sampled data; (0) Do NOT use Log-Sampled Data for Fitting
    ...
    save([pathname sprintf('%s%s-settingsStruct-average.mat',filesep,savePrepend)],'settingsStruct')
    ...
    if length(FilesCheck) > 1
        ...
        switch lower(strSwitch)
            case lower('FD')
                Files=dir([pathname filesep '*.ibw']);
            ...
            case lower({'CytoDCell-MelanocytesExp', 'UCell-MelanocytesExp',...
                ...
                Files=dir([pathname filesep '*.spm']);
                for k=1:length(Files)
                    FileNames = Files(k).name;
                    FileInfo = strsplit(FileNames, {'_' '.'},'CollapseDelimiters',true);
        ...
        switch lower(strSwitch)
            ...
            case lower({'CytoDcell', '1205LuWT', 'CytoDCell-MelanocytesExp',...
                ...
                if useBruker == 1
                    ...
                    %open a file
                    NSMU.Open(fullfile([Files(k).folder filesep Files(k).name]));
                    ...
                    fid = fopen([Files(k).folder filesep Files(k).name],'r');

                    sHSDC_datalength=di_header_find([Files(k).folder filesep Files(k).name],'\Data length:');
                    sHSDC_dataoffset=di_header_find([Files(k).folder filesep Files(k).name],'\Data offset:');
                    ...
                    k_temp = di_header_find([Files(k).folder filesep Files(k).name],'\Spring Constant:');
                    fseek(fid,k_temp(1),-1);
                    line = fgets(fid);
                    k_cantilever(k) = extract_num(line);

                    InVOLS_temp = di_header_find([Files(k).folder filesep Files(k).name],'\@Sens. DeflSens:');
                    fseek(fid,InVOLS_temp(1),-1);
                    line = fgets(fid);
                    defl_InVOLS = extract_num(line); % Value in nm/V

                    InVOLS2_temp = di_header_find([Files(k).folder filesep Files(k).name],'\@Sens. ZsensSens:');
                    fseek(fid,InVOLS2_temp(1),-1);
                    line = fgets(fid);
                    zSens_InVOLS = extract_num(line); % Value in nm/V

                    deflScale_temp = di_header_find([Files(k).folder filesep Files(k).name],'\@4:Z scale: V [Sens. DeflSens]');
                    fseek(fid,deflScale_temp,-1);
                    line = fgets(fid);
                    SensDeflSens_InVOLS = extract_num(line); % Value in V/LSB

                    zScale_temp = di_header_find([Files(k).folder filesep Files(k).name],'\@4:Z scale: V [Sens. ZsensSens]');
                    ...
                    % Make time array
                    frewind(fid);
                    dt_temp = di_header_find([Files(k).folder filesep Files(k).name],'\@Sample period: V (0.1000000 us/LSB)');
    ...
    for iSims = 1:nSims
        ...
        fid = fopen([pathname sprintf('%sFitParams-nBranches_%d.txt',filesep,voigtArray(iSims))],'w');
    ...
    %         grid on
    %         hold off
    %         saveas(hertzPlot,[pathname sprintf('%sHertzEstimate-LoadLevel_%d',filesep,i_loop)],'fig')
    %         saveas(hertzPlot,[pathname sprintf('%sHertzEstimate-LoadLevel_%d',filesep,i_loop)],'jpg')
        ...
        if strcmp(avAnalysis,'a')
            save([pathname sprintf('%s%s-dataStruct-%d_terms-average.mat',filesep,savePrepend,n_terms)],'dataStruct')
            fprintf('\nSaved the dataStruct to an m-file for %d terms and all velocities.\n',n_terms);
        elseif strcmp(avAnalysis,'i')
            save([pathname sprintf('%s%s-dataStruct-%d_terms-individual.mat',filesep,savePrepend,n_terms)],'dataStruct')
    ...
    if timeOperation
        save([pathname sprintf('%s%s-TimerData-%d_terms-%s.mat',filesep,savePrepend,n_terms,avAnalysis)],...
